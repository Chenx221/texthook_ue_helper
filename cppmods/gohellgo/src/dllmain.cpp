...
bool CopyTextToClipboard(const std::wstring& text)
{
    ...
    if (!processed_text)
    {
        Output::send<LogLevel::Warning>(STR("[gohellgo] TextHookPoint returned null, using original text\n"));
        processed_text = text.c_str();
    }

    if (!OpenClipboard(nullptr))
    {
        Output::send<LogLevel::Error>(STR("[gohellgo] Failed to open clipboard\n"));
        return false;
    }
    ...
    {
        CloseClipboard();
        Output::send<LogLevel::Error>(STR("[gohellgo] Failed to empty clipboard\n"));
        return false;
    }
    ...
    {
        CloseClipboard();
        Output::send<LogLevel::Error>(STR("[gohellgo] Failed to allocate memory for clipboard\n"));
        return false;
    }
    ...
    if (auto* pGlobal = static_cast<wchar_t*>(GlobalLock(hGlobal)))
    {
        ...
        if (!SetClipboardData(CF_UNICODETEXT, hGlobal))
        {
            GlobalFree(hGlobal);
            CloseClipboard();
            Output::send<LogLevel::Error>(STR("[gohellgo] Failed to set clipboard data\n"));
            return false;
        }
    }
    else
    {
        GlobalFree(hGlobal);
        CloseClipboard();
        Output::send<LogLevel::Error>(STR("[gohellgo] Failed to lock global memory\n"));
        return false;
    }
...
bool SafeGetFStringProperty(UObject* Context, UClass* ObjectClass, const wchar_t* PropertyName, StringType& OutValue)
{
    ...
    {
        Output::send<LogLevel::Error>(STR("[gohellgo] Exception while getting FString property '{}': {}\n"), PropertyName, ensure_str(e.what()));
        return false;
    }
    catch (...)
    {
        Output::send<LogLevel::Error>(STR("[gohellgo] Unknown exception while getting FString property '{}'\n"), PropertyName);
        return false;
    }
}
...
bool SafeGetArrayProperty(UObject* Context, UClass* ObjectClass, const wchar_t* PropertyName, TArray<FString>** OutArray)
{
    ...
    {
        Output::send<LogLevel::Error>(STR("[gohellgo] Exception while getting array property '{}': {}\n"), PropertyName, ensure_str(e.what()));
        return false;
    }
    catch (...)
    {
        Output::send<LogLevel::Error>(STR("[gohellgo] Unknown exception while getting array property '{}'\n"), PropertyName);
        return false;
    }
}
...
bool SafeGetTextBlockText(UObject* Context, UClass* ObjectClass, const wchar_t* PropertyName, StringType& OutValue)
{
    ...
    {
        Output::send<LogLevel::Error>(STR("[gohellgo] Exception while getting TextBlock text '{}': {}\n"), PropertyName, ensure_str(e.what()));
        return false;
    }
    catch (...)
    {
        Output::send<LogLevel::Error>(STR("[gohellgo] Unknown exception while getting TextBlock text '{}'\n"), PropertyName);
        return false;
    }
}

class gohellgo : public CppUserModBase
{
  private:
    struct HookData
    {
  ...
  public:
    gohellgo()
    {
        ...
        {
            Output::send<LogLevel::Warning>(STR("[gohellgo] Warning: Clipboard access may be limited\n"));
        }
    }
    ...
    ~gohellgo() override
    {
        ...
        Output::send<LogLevel::Warning>(STR("[gohellgo] Unloaded\n"));
    }
    ...
    auto on_unreal_init() -> void override
    {
        if (m_initialized)
        {
            Output::send<LogLevel::Warning>(STR("[gohellgo] Already initialized, skipping\n"));
            return;
        }
        m_initialized = true;

        Output::send<LogLevel::Warning>(STR("[gohellgo] Starting hook registration\n"));
        TryRegisterParseStringHook();
        Output::send<LogLevel::Warning>(STR("[gohellgo] Initial hook registration complete (hook may register later if not found)\n"));
    }
  ...
  private:
    ...
    void UnregisterHook(HookData& hook)
    {
        if (hook.registered)
        {
            try
            {
                ...
                Output::send<LogLevel::Verbose>(STR("[gohellgo] Unregistered hook: {}\n"), hook.target_name);
            }
            catch (const std::exception& e)
            {
                Output::send<LogLevel::Error>(STR("[gohellgo] Error unregistering hook: {}\n"), ensure_str(e.what()));
            }
        }
    }
    ...
    static void ApplyTextFilters(StringType& combinedText)
    {
        ...
        {
            Output::send<LogLevel::Error>(
                STR("[gohellgo] Exception during filtering: {}\n"),
                ensure_str(e.what())
            );
        }
    }
    ...
    // 提取并合并所有文本
    static void ExtractAndCopyText(UObject* Context, gohellgo* modInstance)
    {
        // 测试用
        // Output::send<LogLevel::Verbose>(STR("[gohellgo] hit, but nothing to do\n"));
        // return;
        if (!Context)
        {
            Output::send<LogLevel::Verbose>(STR("[gohellgo] ExtractAndCopyText: Context is null\n"));
            return;
        }
        ...
        if (!ObjectClass)
        {
            Output::send<LogLevel::Verbose>(STR("[gohellgo] ExtractAndCopyText: Failed to get class\n"));
            return;
        }
        ...
        if (SafeGetArrayProperty(Context, ObjectClass, STR("TextArray"), &MainStringArray))
        {
            if (MainStringArray && MainStringArray->Num() > 0)
            {
                ...
                Output::send<LogLevel::Verbose>(STR("[gohellgo] TextArray: {}\n"), combinedText);
            }
        }
        ...
        if (SafeGetTextBlockText(Context, ObjectClass, STR("TargetName_Box"), textBlockText))
        {
            ...
            {
                Output::send<LogLevel::Verbose>(STR("[gohellgo] TargetName_Box: {}\n"), textBlockText);
        ...
        if (!combinedText.empty())
        {
            if (modInstance)
            {
                ...
                {
                    Output::send<LogLevel::Verbose>(STR("[gohellgo] Text is same as previous, not copying\n"));
                    return;
                }
            ...
            Output::send<LogLevel::Warning>(STR("[gohellgo] Extracted text: {}\n"), combinedText);

            if (!CopyTextToClipboard(combinedText))
            {
                Output::send<LogLevel::Error>(STR("[gohellgo] Failed to copy text to clipboard\n"));
            }
        }
        else
        {
            Output::send<LogLevel::Verbose>(STR("[gohellgo] No text extracted\n"));
        }
    }
    ...
    void TryRegisterParseStringHook()
    {
        ...
        if (m_setup_text_hook.retry_count > HookData::max_retries)
        {
            ...
            Output::send<LogLevel::Warning>(STR("[gohellgo] Setup_Text hook registration failed after {} retries, skipping\n"), HookData::max_retries);
            return;
        }
        ...
        try
        {
            ...
            if (!Function)
            {
                ...
                {
                    Output::send<LogLevel::Verbose>(STR("[gohellgo] Setup_Text function not found yet, will retry\n"));
                }
                return;
            }
            ...
            auto post_callback = [](UnrealScriptFunctionCallableContext& context, void* custom_data) -> void {
                try
                {
                    auto* mod_instance = static_cast<gohellgo*>(custom_data);
                    ExtractAndCopyText(context.Context, mod_instance);
                }
                catch (const std::exception& e)
                {
                    Output::send<LogLevel::Error>(STR("[gohellgo] Exception in Setup_Text PostHook: {}\n"), ensure_str(e.what()));
                }
                catch (...)
                {
                    Output::send<LogLevel::Error>(STR("[gohellgo] Unknown exception in Setup_Text PostHook\n"));
                }
            };
            ...
            m_setup_text_hook.should_retry = false;
            Output::send<LogLevel::Warning>(STR("[gohellgo] Registered Setup_Text hook (retry: {})\n"), m_setup_text_hook.retry_count);
        }
        catch (const std::exception& e)
        {
            Output::send<LogLevel::Error>(STR("[gohellgo] Failed to register Setup_Text hook: {}\n"), ensure_str(e.what()));
        }
        catch (...)
        {
            Output::send<LogLevel::Error>(STR("[gohellgo] Failed to register Setup_Text hook: Unknown error\n"));
        }
    }
};
